...
#define MESSAGE_DELIMITER '$'  // 메세지 구분 기호(가변 길이 메세지에 대한 메세지 종료 기호)
#define FIELD_DELIMITER '#'   // 필드 구분 문자

// 역직렬화할 때 메세지 구분기호($)를 찾기 위해서 링버퍼를 사용하기 위한 구조체
...
// 서버측 : 클라이언트의 요청을 역직렬화하는 함수(클라이언트로부터 받음)
...
// 서버측 : 클라이언트의 요청에대한 응답을 직렬화하는 함수(직렬화해서 클라이언트로 보냄)
